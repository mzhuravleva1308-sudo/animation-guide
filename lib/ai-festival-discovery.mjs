...
import {
  formatResonaleMajorFestivalsForPrompt,
...
/** @deprecated Prefer RESONALE_MAJOR_FESTIVALS / formatResonaleMajorFestivalsForPrompt */
export const AI_DISCOVERY_MAJOR_FESTIVALS = RESONALE_MAJOR_FESTIVALS.map(
...
export function buildAiFestivalDiscoveryPrompt(film, options = {}) {
  const festivalScope = options.festivalFilterId
    ...
    : `Check whether this animated film participated at ANY of these Resonale major festivals ONLY (premiere, competition, official selection, or listed major sidebar). Do NOT include any festival outside this list:
${formatResonaleMajorFestivalsForPrompt()}`;
...
Rules:
- Return one entry per Resonale major festival where participation is plausible.
- Do NOT return festivals outside the Resonale major list (Oscar-qualifying alone is not enough).
- Count ONLY this film's screening, premiere, or official/sidebar selection in a festival program (Competition, Un Certain Regard, Directors' Fortnight, Official Selection, Critics' Week, etc.).
- Do NOT count Marché du Film, Shorts Corner, market pitches, WIP, producer showcases, or a director's other projects as festival participation for this film.
...
- Return an empty festivals array when there is no plausible major-festival participation.
...
function parseAiFestivalItem(item, options = {}) {
  ...
  if (
    ...
    !isResonaleMajorFestival(festivalName, {
      original_text: item.reason ?? item.original_text ?? item.originalText,
    ...
    input: [
      {
        role: "system",
        content:
          "You identify plausible festival participation for animated films from public knowledge. Count ONLY Resonale major festivals from the provided list. Include Cannes Official Selection / Un Certain Regard / Critics' Week / Directors' Fortnight when the film itself screened there — not Marché du Film, Shorts Corner, pitch, or WIP. Return only JSON. Never invent awards or nomination details.",
      },
      {
        role: "user",
        content: prompt,
