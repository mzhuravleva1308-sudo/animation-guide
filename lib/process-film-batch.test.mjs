...
/** Avoids loading rebuild-profile-film-scores.mjs (needs SERVICE_ROLE_KEY). */
function mockScoreFilms() {
  let calls = 0;
  /** @type {string[][]} */
  const filmIdCalls = [];
  const scoreFilms = async (filmIds) => {
    calls += 1;
    filmIdCalls.push([...(filmIds ?? [])]);
    return {
      profileCount: 12,
      rowCount: (filmIds?.length ?? 0) * 12,
      emptyProfiles: 0,
    };
  };
  return {
...
test("duplicate of one film does not block the others", async () => {
  ...
  assert.equal(scores.calls, 1);
  assert.equal(scores.filmIdCalls[0].length, 1);
...
test("19 successful and 1 failed score only successful film ids once", async () => {
  const supabase = new FakeSupabase();
  ...
  assert.equal(result.enqueueCallCount, 1);
  assert.equal(scores.calls, 1);
  assert.equal(scores.filmIdCalls[0].length, 19);
  assert.deepEqual(scores.filmIdCalls[0], result.successfulFilmIds);
  assert.equal(result.incrementalScores.rowCount, 19 * 12);
  assert.equal(result.exitCode, EXIT.PARTIAL);
});
...
test("incremental scoring is called once for mixed success", async () => {
  const supabase = new FakeSupabase();
  const scores = mockScoreFilms();
  ...
  assert.equal(scores.calls, 1);
  assert.equal(scores.filmIdCalls[0].length, 2);
});

test("discovery_release defers profile score enqueue", async () => {
  const supabase = new FakeSupabase();
  const scores = mockScoreFilms();
  const result = await processFilmImportBatch({
    supabase,
    batch: importBatch([
      makeFilmInput({
        ...
        year: 2016,
        origin: "discovery_release",
        catalog_visible: false,
        discovery_candidate_id: "2829cfb3-063c-4435-9b28-24dbed212231",
        moods: ["dark"],
        aesthetic_tags: ["sketch-like"],
      }),
    ]),
    options: { dryRun: false, execute: true },
    pipeline: successPipeline(),
  ...
  assert.equal(scores.calls, 0);
  assert.equal(result.enqueueCalled, false);
  assert.equal(result.results[0].deferProfileEnqueue, true);
  assert.equal(result.results[0].checklist.profile_scores, "deferred");
});

test("scoring is not called when every film fails", async () => {
  const supabase = new FakeSupabase();
  const scores = mockScoreFilms();
  const result = await processFilmImportBatch({
    supabase,
...
test("import persists quick_filters on the films row", async () => {
  ...
  await processFilmImportBatch({
    ...
    options: { dryRun: false, execute: true },
    pipeline: successPipeline(),
    scoreFilms: mockScoreFilms().scoreFilms,
  });
...
test("downstream failure rolls back before scoring", async () => {
  const supabase = new FakeSupabase();
  const scores = mockScoreFilms();
  const result = await processFilmImportBatch({
    supabase,
