 ...
 *   sci-fi | sarcasm | connection (Light) | distance (Shadow)
...
/**
 * Human-readable labels for stored manual quick_filters tokens.
 * @param {unknown} value
 * @returns {string[]}
 */
export function formatDiscoveryQuickFilterLabels(value) {
  return normalizeDiscoveryQuickFilters(value).map(
    (token) =>
 ...
 * new Light/Shadow/Sarcasm criteria.
 */
export function buildQuickFiltersPromptSection() {
  return `
CATALOG quick_filters (manual tokens only — same as public Sci-Fi / Sarcasm / Light / Shadow):
Allowed tokens ONLY: "sci-fi", "sarcasm", "connection", "distance".
- "sci-fi": stories shaped by technology, space, imagined futures (same meaning as the public Sci-Fi chip).
- "sarcasm": Darkly funny films with dry irony, cynicism and bite. Require a clear sarcastic / caustic / black-humor-with-edge tone. Do NOT use for merely absurd, chaotic, quirky, or dark comedies without that bite.
...
- sci-fi and sarcasm may stand alone or combine with connection OR distance (and with each other).
