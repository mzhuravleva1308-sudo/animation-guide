...
- Do NOT invent other tokens. Do NOT store stop-motion / recent / festival / award-winners here (those are derived: stop-motion from technique, recent from year, festival from has_festival/claims, award-winners from festival_recognitions).
...
export function buildDiscoveryCatalogFilterPills(film) {
  ...
  // Festival filter: participation yes/no (claims, flag, or award win ⇒ participated).
  const hasFestivalFlag =
    film.has_festival === true ||
    (Array.isArray(film.festival_claims) && film.festival_claims.length > 0) ||
