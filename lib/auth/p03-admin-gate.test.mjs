...
describe("P0-3 import-film and check-duplicate gate order", () => {
  ...
  it("festival page gates before service-role query and admin pages are protected uniformly", () => {
    ...
    const releases = fs.readFileSync(
      path.join(root, "app/admin/film-releases/page.tsx"),
      "utf8"
    );
    assert.match(releases, /getAdminAccessStatus/);
    assert.match(releases, /redirect\("\/login"\)/);
    assert.match(releases, /notFound\(\)/);
  });
});
