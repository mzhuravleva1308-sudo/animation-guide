  ...
  const supabase = {
    ...
    from(table) {
      return {
        ...
        insert(row) {
          // Phase A: share_token is no longer set by runtime.
...
describe("ensureAuthProfileForUser", () => {
  ...
  it("returns profile with null share_token as linked (Phase A: no repair)", async () => {
    const supabase = createMockSupabase({
      profileByUserId: {
        id: "profile-1",
        slug: "maria",
  ...
  it("creates a new profile when no linked profile exists", async () => {
    ...
    assert.deepEqual(
      Object.keys(supabase.calls.find((call) => call.type === "insert")?.payload ?? {}).sort(),
      ["slug", "user_id"]
    );
  ...
  it("retries a duplicate slug and succeeds with the next candidate", async () => {
    const supabase = createMockSupabase({
      profileByUserId: null,
      insertResults: [
        {
    ...
    assert.equal(result.created, true);
    assert.equal(result.profile.slug, "new-user-user-new-2");
    assert.equal(supabase.calls.filter((call) => call.type === "insert").length, 2);
  });

  it("returns the database error code when profile creation fails", async () => {
    const supabase = createMockSupabase({
      profileByUserId: null,
      insertResults: [
        {
    ...
    await assert.rejects(
      ensureAuthProfileForUser(supabase, {
        id: "user-new",
        email: "new.user@example.com",
      }),
      (error) => {
