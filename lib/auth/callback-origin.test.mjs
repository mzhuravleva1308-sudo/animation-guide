...
describe("sanitizeNextPath", () => {
  it("allows relative in-app paths", () => {
    ...
    assert.equal(
      sanitizeNextPath("/?media=live_action&filter=sci-fi"),
      "/?media=live_action&filter=sci-fi"
    );
  });
...
describe("resolveAuthOriginFromRequest", () => {
  ...
  it("prefers the Host header over a mismatched request URL origin", () => {
    const request = new Request("http://localhost:3100/auth/callback?token_hash=abc&type=email", {
      headers: { host: "127.0.0.1:3100" },
    });
    assert.equal(
      resolveAuthOriginFromRequest(request),
      "http://127.0.0.1:3100"
    );
  });
});
