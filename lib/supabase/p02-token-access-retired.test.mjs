...
describe("P0-2 /api/profile-rating and /api/profile-save require session (local Supabase)", () => {
  ...
  it("unauthenticated POST to /api/profile-activity with foreign profileId → 401", async (t) => {
    if (skip(t, "API session auth test")) return;

    const appOrigin = process.env.NEXT_PUBLIC_SITE_URL?.replace(/\/$/, "") ?? "http://localhost:3000";

    const res = await fetch(`${appOrigin}/api/profile-activity`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        profileId: "00000000-0000-0000-0000-000000000001",
        eventType: "rating_set",
        filmId: "00000000-0000-0000-0000-000000000002",
      }),
    }).catch(() => null);
