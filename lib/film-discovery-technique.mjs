...
export function resolveTechniqueStatusPolicy(input) {
  ...
  if (labels.length > 0 && withDirectEvidence === 0) {
    // Safety net: inferred labels should normally be stripped before this runs.
    techniqueNotes.push(
      "Technique labels lack a direct production-method citation; left for manual review."
    );
  }
  ...
  for (const row of provenance) {
    ...
    if (/TMDB keyword/i.test(String(row.sourceLabel ?? ""))) {
      techniqueNotes.push(
        `Technique "${row.label}" from TMDB keyword; verify before approval.`
      );
    }
    if (row.sourceTier === "ai" || row.ai || /AI technique/i.test(String(row.sourceLabel ?? ""))) {
      techniqueNotes.push(
        `Technique "${row.label}" suggested by AI; verify before approval.`
      );
    }
  }
...
/**
 * Prefer evidence-backed labels only; keep at most 2.
 * Never persist proposed labels that lack matching research evidence.
 * When any distinctive label is kept, drop generic siblings.
 *
...
export function preferEvidenceBackedTechniqueLabels(
  ...
  if (!evidenceLabels.length) return [];

  const proposedNormalized = (proposedLabels ?? [])
    .map((label) => {
      const classified = classifyTechniqueLabel(label);
      if (classified.kind === "synonym" || classified.kind === "canonical") {
  ...
  const push = (label) => {
    if (!label || merged.length >= 2) return;
    const key = label.toLowerCase();
    if (mergedSeen.has(key)) return;
    // Only evidence-backed labels.
    const evidenced = evidenceLabels.some((item) => item.toLowerCase() === key);
    if (!evidenced) return;
    mergedSeen.add(key);
    merged.push(label);
  };

  // Distinctive evidence first (whether or not curator proposed it).
  for (const label of evidenceLabels) {
    if (isDistinctiveTechniqueLabel(label)) push(label);
  }
  // Then proposed labels that match evidence (distinctive already added).
  for (const label of proposedNormalized) {
    push(label);
  }
  // Then remaining evidence (basic technique only when research cited it).
  for (const label of evidenceLabels) {
    push(label);
  }

  const hasDistinctive = merged.some((label) => isDistinctiveTechniqueLabel(label));
  if (hasDistinctive) {
    return merged.filter((label) => !isGenericTechniqueLabel(label)).slice(0, 2);
  }
