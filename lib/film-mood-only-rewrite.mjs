...
export function buildMoodOnlyWriterPrompt(film, guide, overused = []) {
  ...
  const techRaw = film.technique;
  const techStr = Array.isArray(techRaw)
    ? techRaw.join(" ")
    : String(techRaw ?? "");
  const common2d =
    /\b2\s*d\b/i.test(techStr) && !/\b(stop|roto|puppet|cgi|3\s*d)\b/i.test(techStr);

  return `
You are the Resonale Content writer, mood-only mode.
Write ONLY the_mood for this film using the Resonale Mood Writing Guide.
Do NOT change synopsis, technique, or moods tags.
Do NOT invent plot facts. Do NOT retell the synopsis.

Voice: a thoughtful friend helping someone quickly understand what watching feels like.

${formatMoodWritingGuideForPrompt(guide)}

Operational rules for this write:
- Name the mood in the first few words.
- Then add 1–2 concrete film-owned cues (place, image, sound, behavior, distinctive contrast).
- Default shape is fine: "[Mood], with [concrete specifics]."
- Adj openings and repeated shapes across films are fine. Variation is not a goal.
- Do NOT use pacing/atmosphere/moments/tension as default filler.
${
  common2d
    ? `- This film’s listed technique is ordinary 2D animation shared by many films. Do NOT write "with … 2D animation". Prefer setting/image/behavior instead.`
    : `- Mention technique only if it is distinctive and shapes the feel (stop-motion puppets, rotoscope, stark black-and-white, unusual hybrid).`
}
- No literary critic prose.
...
Title: ${film.title}
Year: ${film.year ?? ""}
Technique (context only): ${JSON.stringify(film.technique ?? null)}
Synopsis (context only — do not paraphrase into mood):
${film.synopsis ?? ""}
Moods tags: ${JSON.stringify(film.moods ?? [])}
Previous the_mood (keep only if it already passes the guide; otherwise rewrite):
${film.previous_the_mood ?? film.the_mood ?? ""}

HARD LIMIT: the_mood must be at most ${EDITORIAL_COPY_LIMITS.the_mood} words (UI budget). Prefer 12–20.
...
export async function rewriteMoodForFilm(film, options = {}) {
  ...
  if ((validation.issues.length || !theMood) && options.openai && !options.skipFixRevision) {
    ...
    if (retriedMood && !reval.issues.length) {
      ...
      const previousIssues = validateMoodOnly(previous, film.synopsis ?? "").issues;
      // Prefer a fresh draft over keeping a previous mood that also fails guide validation.
      if (previous && !previousIssues.length) {
        theMood = previous;
        unchanged = true;
        writerNote = `validation_failed_kept_previous: ${validation.issues.join("; ")}`;
      } else {
        theMood = cleanupEditorialField(retriedMood || theMood || previous);
        unchanged = theMood === previous;
        writerNote = `validation_soft_kept_new: ${[...validation.issues, ...reval.issues].join("; ")}`;
      }
    }
    ...
    if (previous && !previousIssues.length) {
      ...
      theMood = cleanupEditorialField(theMood || previous);
      unchanged = theMood === previous;
      writerNote = `validation_soft_kept_new: ${validation.issues.join("; ")}`;
    }
  } else {
