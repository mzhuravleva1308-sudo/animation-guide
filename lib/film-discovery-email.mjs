...
export function formatWeeklyFilmDiscoveryEmail(input) {
  ...
  if (input.passed.length === 0) {
    ...
    input.passed.forEach((film, index) => {
      ...
      lines.push(`   Content status: ${film.content_status ?? "pending"}`);
      if (film.media_notes) {
        lines.push(`   Media notes: ${film.media_notes}`);
      }
      if (film.content_note) {
        lines.push(`   Content note: ${film.content_note}`);
      }
  ...
  lines.push("");
  lines.push(
    "Next step: open /admin/film-discovery to review poster, trailer, synopsis, mood, technique, and Approve or Reject."
  );
  lines.push(
    "Approve does NOT publish, enrich further, or write to the public catalog."
  );
