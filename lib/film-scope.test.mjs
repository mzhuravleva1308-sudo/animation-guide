...
import {
  RANKING_FILM_IDS,
  assertExactRankingFilmIds,
} from "../scripts/ranking-film-scope.mjs";
...
describe("ranking film scope", () => {
  it("accepts exactly the five approved UUIDs in any order", () => {
    assert.deepEqual(
      assertExactRankingFilmIds([...RANKING_FILM_IDS].reverse()),
      RANKING_FILM_IDS
    );
  });

  it("rejects missing, extra, or duplicate UUIDs", () => {
    assert.throws(
      () => assertExactRankingFilmIds(RANKING_FILM_IDS.slice(1)),
      /scope mismatch/
    );
    assert.throws(
      () => assertExactRankingFilmIds([...RANKING_FILM_IDS, "extra"]),
      /scope mismatch/
    );
    assert.throws(
      () => assertExactRankingFilmIds([RANKING_FILM_IDS[0], ...RANKING_FILM_IDS]),
      /scope mismatch/
    );
  });
});
