...
export function validateDiscoveryContentDraft(draft, options = {}) {
  ...
  const moodNormalized = normalizeDiscoveryMoodTagsPreferCatalog(draft.moods);
  const moods = moodNormalized.moods;
  if (
    moods.length &&
    (moods.length < MOOD_TAGS_MIN || moods.length > MOOD_TAGS_MAX)
  ) {
    softNotes.push(
      `moods should ideally have ${MOOD_TAGS_MIN}–${MOOD_TAGS_MAX} tags (got ${moods.length})`
    );
  }
  if (moodNormalized.offVocabulary.length >= 3) {
    softNotes.push(
      `moods include off-vocabulary tags: ${moodNormalized.offVocabulary
        .slice(0, 5)
        .join(", ")}`
    );
  }
...
- moods: ${MOOD_TAGS_MIN}–${MOOD_TAGS_MAX} emotional tags in AI catalog style (same as fill-emotional-tags); prefer catalog vocabulary; no technique/genre as mood. Tags and the_mood sentence complement each other.
${buildMoodTagsPromptSection()}
- aesthetic_tags: ${AESTHETIC_TAGS_MIN}–${AESTHETIC_TAGS_MAX} material/visual-feeling tags in AI catalog style (same as fill-aesthetic-tags); not moods, not plain technique labels.
...
Return ONLY JSON:
{
  ...
  "the_mood": "...",
  "technique": ["label1"],
  "moods": ["tender", "bittersweet", "hopeful", "intimate"],
  "aesthetic_tags": ["lush hand-drawn world", "tactile textures", "organic textures", "storybook-like"],
  "quick_filters": ["sci-fi", "sarcasm", "distance"],
