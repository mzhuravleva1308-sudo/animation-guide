...
export function validateDiscoveryContentDraft(draft, options = {}) {
  ...
  let labels = tech.labels;
  if ((options.techniqueEvidence ?? []).length) {
    const preferred = preferEvidenceBackedTechniqueLabels(
      tech.labels,
      options.techniqueEvidence
    );
    if (preferred.length) labels = preferred;
  }
  // Keep basic writer proposal when research is empty.
  if (!labels.length && tech.labels.length) labels = tech.labels;
  ...
  const techPolicy = resolveTechniqueStatusPolicy({
    ...
    techniqueEvidence: options.techniqueEvidence ?? [],
...
- technique: max 2 production-method labels. Basic technique (e.g. 2D animation, stop-motion) is OK with reasonable confidence. Put doubts in technique_notes instead of leaving technique empty.
...
Do NOT FIX only because synopsis mainly covers the setup, or basic technique lacks an official source.

Style guide version: ${guide.version}
Mood Writing Guide version: ${moodGuideVersion}

For the_mood, judge against the Mood Writing Guide:
...
- PASS_WITH_NOTE — usable, but leave a concrete non-blocking note (TMDB-heavy synopsis, secondary technique source, slightly generic wording, possible distinctive technique to verify, mild mood template feel, a repeated word).
- FIX — only for significant problems: factual error, spoiler, broken English, clear ad copy, synopsis/mood substantial repetition, clearly wrong technique, non-technique label, unreadable text, excessive length, misleading unsupported specific claim.
- For the_mood specifically, FIX only if: clearly generic / interchangeable across unrelated films, unnatural English, repeats synopsis, unconfirmed fact, near-verbatim copy of a guide example, or clear guide violation. Do NOT FIX for a single shared construction with another film.
...
export async function runDiscoveryContentBatch(candidates, options = {}) {
  ...
  const wikipediaState =
    options.wikipediaState ?? createWikipediaResearchState({ delayMs: 900 });
  const curateFn =
    options.curateFn ??
    ((candidate) =>
