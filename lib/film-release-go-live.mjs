/**
 * Batch Go Live: catalog_visible=true for selected prepped films +
 * incremental profile scores for those films only (likes still use full rebuild jobs).
 */
...
/** Lazy so callers/tests can load this module without SERVICE_ROLE_KEY. */
async function defaultScoreNewFilms(filmIds, options = {}) {
  const { scoreNewFilmsForAllProfiles } = await import(
    "../scripts/rebuild-profile-film-scores.mjs"
  );
  return scoreNewFilmsForAllProfiles(filmIds, options);
}

/**
 * @deprecated Full rebuild enqueue — kept for --rebuild-all-profiles / manual ops.
 * Go live and new-film import use scoreNewFilmsForAllProfiles instead.
 */
 ...
 * @param {{ actor?: string | null, notes?: string | null, supabaseUrl?: string, scoreFilms?: Function }} [options]
 */
export async function goLiveFilmBatch(supabase, filmIds, options = {}) {
  ...
  const scoreFilms = options.scoreFilms ?? defaultScoreNewFilms;
  const scoreSummary = await scoreFilms(ids);
  ...
  return {
    ...
    profileJobs: 0,
    incrementalScores: scoreSummary,
    candidateIds,
  };
}
