...
import {
  ...
  formatDiscoveryQuickFilterLabels,
  isRecentFilmYear,
...
test("normalizeDiscoveryQuickFilters keeps closed vocabulary", () => {
  assert.deepEqual(
    normalizeDiscoveryQuickFilters(["Sci-Fi", "Sarcasm", "connection", "bogus"]),
    ["sci-fi", "sarcasm", "connection"]
  );
});

test("formatDiscoveryQuickFilterLabels uses UI names", () => {
  assert.deepEqual(
    formatDiscoveryQuickFilterLabels(["sarcasm", "distance", "sci-fi"]),
    ["Sarcasm", "Shadow", "Sci-Fi"]
  );
});
