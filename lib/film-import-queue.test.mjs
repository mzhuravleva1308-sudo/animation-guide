...
test("batch continues after one film fails", async () => {
  ...
  const statusUpdates = updates.filter((row) => row.payload?.status != null);
...
test("processClaimedQueueItem isolates one film payload for the pipeline", async () => {
        ...
        results: [
          {
            ...
            status: PIPELINE_STATUS.CATALOG_READY,
            filmId: "f-1",
            checklist: { inserted: true, poster_cached_storage: true },
          },
        ],
      };
    },
  ...
  assert.equal(result.checklist.inserted, true);
  assert.ok(updates.some((row) => row.table === "film_import_queue"));
});

test("processFilmImportQueueItemById claims pending row then processes", async () => {
  const { processFilmImportQueueItemById } = await import(
    "./film-import-queue.mjs"
  );
  const item = makeItem({
    id: "q-approve",
    title: "Fresh",
  ...
  const supabase = {
    from(table) {
      assert.equal(table, "film_import_queue");
      return {
        select() {
          return {
            eq() {
        ...
        update(payload) {
          return {
            eq() {
              return {
                eq() {
                  // claim path ends with select().maybeSingle()
                  // mark path awaits the second eq() directly
                  const maybeSingle = async () => {
                    stored = {
                      ...stored,
                      ...payload,
                      attempts:
                        payload.status === QUEUE_STATUS.PROCESSING
                          ? (Number(stored.attempts) || 0) + 1
                          : stored.attempts,
                    };
                    return { data: { ...stored }, error: null };
                  };
                  const result = Promise.resolve({ error: null }).then(() => {
                    if (payload.status !== QUEUE_STATUS.PROCESSING) {
                      marks.push(payload);
                      stored = { ...stored, ...payload };
                    }
                    return { error: null };
                  });
                  result.select = () => ({ maybeSingle });
                  // When mark awaits second eq, resolve after recording
                  if (payload.status !== QUEUE_STATUS.PROCESSING) {
                    marks.push(payload);
                    stored = { ...stored, ...payload };
                    return Promise.resolve({ error: null });
  ...
  assert.equal(result.queueUpdate.status, QUEUE_STATUS.COMPLETED);
  assert.equal(result.queueUpdate.film_id, "film-fresh");
