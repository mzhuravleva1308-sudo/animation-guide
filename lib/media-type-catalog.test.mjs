...
describe("live-action catalog access", () => {
  it("is open to every viewer (allowlist retired)", () => {
    assert.equal(canAccessLiveActionCatalog("someone@else.com"), true);
    assert.equal(canAccessLiveActionCatalog(null), true);
    assert.equal(canAccessLiveActionCatalog(""), true);
    assert.deepEqual(LIVE_ACTION_CATALOG_ALLOWLIST_EMAILS, []);
  });
});
