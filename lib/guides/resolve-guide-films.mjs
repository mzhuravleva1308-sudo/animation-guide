...
/**
 * @typedef {{ title: string }} GuideFilmEntry
 ...
 * @param {T[]} films
 * @returns {{
 *   groups: { heading: string, description: string, items: { film: T }[] }[],
...
export function resolveGuideFilms(guide, films) {
  ...
  /** @type {string[]} */
  const missingTitles = [];
  /** @type {{ heading: string, description: string, items: { film: T }[] }[]} */
  const groups = [];
...
export function resolveGuideAnchorFilm(guide, films) {
  const title = String(guide?.anchorTitle ?? "").trim();
  if (!title) {
    return null;
  }

  return (films ?? []).find((film) => film?.title === title) ?? null;
}
