...
/**
 * Media curator status on film_discovery_candidates (independent of review_status).
 */
export const DISCOVERY_MEDIA_STATUS = Object.freeze({
  pending: "media_pending",
  complete: "media_complete",
  partial: "media_partial",
  failed: "media_failed",
  needsReview: "media_needs_review",
});

/** Max Media curator attempts per candidate (initial + one retry). */
export const DISCOVERY_MEDIA_MAX_ATTEMPTS = 2;

/**
 * Content writer status on film_discovery_candidates (independent of review_status / media_status).
 * Final editorial review is manual — statuses mark draft readiness, not publishability.
 * Values are intentionally short (no content_ prefix).
 */
export const DISCOVERY_CONTENT_STATUS = Object.freeze({
  pending: "pending",
  ready: "ready",
  readyWithNote: "ready_with_note",
  /** @deprecated alias — prefer readyWithNote */
  readyWithNotes: "ready_with_note",
  failed: "failed",
});

export const DISCOVERY_CONTENT_VERDICT = Object.freeze({
  pass: "PASS",
  passWithNote: "PASS_WITH_NOTE",
  fix: "FIX",
  /** @deprecated alias — treat as FIX in legacy paths */
  revise: "FIX",
});

/** Max content revisions after reviewer FIX (hard cap = 1). */
export const DISCOVERY_CONTENT_MAX_REVISIONS = 1;
