...
function buildVisualWorldText(film) {
  const tags = (film.visual_world_tags ?? [])
    .map(normalizeTag)
    .filter(Boolean)
    .sort();
  return `live-action film visual world: ${tags.join(", ")}`;
}
...
async function getFilms() {
  return loadScopedFilms(supabase, scope, {
    select: "id, title, visual_world_tags, media_type",
    applyFilters: (query) =>
      query
        .eq("media_type", MEDIA_TYPE.liveAction)
        .not("visual_world_tags", "is", null)
        .order("title"),
  }).then((films) => films.filter((film) => film.visual_world_tags?.length));
}

async function getExistingEmbeddings() {
  const { data, error } = await supabase
    .from("film_visual_world_embeddings")
    .select("film_id, visual_world_text, embedding");
  if (error) throw error;

  const map = new Map();
  for (const row of data ?? []) {
    map.set(row.film_id, {
      text: row.visual_world_text,
...
async function main() {
  ...
  console.log(`Films with visual world tags: ${films.length}`);

  for (const film of films) {
    const text = buildVisualWorldText(film);
    ...
    if (dryRun) {
      console.log(`[dry-run] would create visual world embedding: ${film.title}`);
      console.log(`  ${text}`);
      continue;
    }

    console.log(`Creating visual world embedding: ${film.title}`);
    console.log(`  ${text}`);
    const embedding = await createEmbedding(text);

    const { error } = await supabase.from("film_visual_world_embeddings").upsert(
      {
        film_id: film.id,
